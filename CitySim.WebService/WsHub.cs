...
/// </summary>
public sealed class WsHub(ILogger<WsHub> logger)
{
    ...
    public async Task HandleConnectionAsync(WebSocket ws, CancellationToken ct)
    {
        ...
        logger.LogDebug("[WS]  client connected  (total={Count})", _clients.Count);

        // Send snapshot immediately on connect.
        if (GetSnapshot?.Invoke() is { } snapshot)
        {
            logger.LogDebug("[EVT] {Summary}", Summary(snapshot));
        ...
        finally
        {
            ...
            logger.LogDebug("[WS]  client disconnected (total={Count})", _clients.Count);
    ...
    public void Broadcast(SimEvent evt)
    {
        ...
        logger.LogDebug("[EVT] {Summary}", Summary(evt));
        foreach (var (_, (socket, sendLock)) in _clients)
        {
            ...
            _ = SendAsync(socket, sendLock, payload, CancellationToken.None)
                  .ContinueWith(t =>
                      logger.LogWarning("[WS]  send error: {Message}", t.Exception?.GetBaseException().Message),
